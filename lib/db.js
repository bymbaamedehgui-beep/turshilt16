...
export async function createExam(exam) {
  ...
  const rows = await sql`
    INSERT INTO exams (id,title,subject,sec1_count,sec1_key,sec1_scores,topics,use_sec2,sec2_config,sec2_score,created_at)
    VALUES (${exam.id},${exam.title},${exam.subject},${exam.sec1Count},
...
export async function updateExam(id, exam) {
  ...
  const rows = await sql`
    UPDATE exams SET
      title=${exam.title}, subject=${exam.subject}, sec1_count=${exam.sec1Count},
...
export async function createStudent(s) {
  const sql = getDb();
  const rows = await sql`
    INSERT INTO students (id,exam_id,name,code,version,correct,wrong,blank,
      raw_earned,raw_max,scaled,grade,sec1_results,sec2_results,needs_review,submitted_at)
    VALUES (${s.id},${s.examId},${s.name},${s.code},${s.version||''},
...
export async function getStudentAccounts() {
  const sql = getDb();
  return await sql`SELECT * FROM student_accounts ORDER BY created_at DESC`;
}

export async function createStudentAccount(acc) {
  const sql = getDb();
  const rows = await sql`
    INSERT INTO student_accounts (id,code,name,class,created_at)
    VALUES (${acc.id},${acc.code},${acc.name||''},${acc.class||''},${acc.createdAt||new Date().toISOString()})
    RETURNING *`;
  return rows[0];
}
...
function dbToExam(r) {
  return { id:r.id,title:r.title,subject:r.subject,sec1Count:r.sec1_count,
...
function dbToStudent(r) {
  return { id:r.id,examId:r.exam_id,name:r.name,code:r.code,version:r.version,
