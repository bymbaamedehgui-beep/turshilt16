...
const PAGE_IDS = ['home','create','material','upload','board','analytics','rating','students'];
...
const T = {
  mn: {
    ...
    hello:'Сайн байна уу,',
    nav:{home:'Нүүр',create:'Шалгалт үүсгэх',material:'Материал',upload:'Зураг оруулах',board:'Жагсаалт',analytics:'Анализ',rating:'Рейтинг',students:'Сурагчид'},
  },
  en: {
    appName:'EYESH Checker',
    ...
    hello:'Hello,',
    nav:{home:'Home',create:'Create Exam',material:'Material',upload:'Upload',board:'Results',analytics:'Analytics',rating:'Ranking',students:'Students'},
  }
};
...
function exportExcel(exam, students) {
  import('xlsx').then(XLSX => {
    ...
    XLSX.utils.book_append_sheet(wb,XLSX.utils.aoa_to_sheet([
      ['#','Нэр','Код','Хув.','Зөв','Буруу','Хоосон','Raw','ЭЕШ','Зэрэглэл'],
...
function LoginPage({onLogin, onStudentLogin}) {
  ...
  async function handleTeacherLogin() {
    ...
    try {
      const {token} = await fetch('/api/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:user,password:pass})}).then(async r=>{
        const d=await r.json();
        if(!r.ok) throw new Error(d.error||'Алдаа');
        return d;
      });
      localStorage.setItem('eyesh_token', token);
      onLogin();
  ...
  return (
    <div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'linear-gradient(135deg,#7f1d1d,#dc2626)',padding:20}}>
      <div style={{background:'white',borderRadius:20,padding:'36px 32px',width:'100%',maxWidth:400,boxShadow:'0 20px 60px rgba(0,0,0,.3)'}}>
        ...
        {!role&&(
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
            <button onClick={()=>{setRole('teacher');setErr('');}}
              ...
              <div style={{fontWeight:800,fontSize:15,color:'#1e293b'}}>{t.teacher}</div>
              <div style={{fontSize:11,color:'#94a3b8',marginTop:4}}>{t.loginWithPass}</div>
            </button>
            <button onClick={()=>{setRole('student');setErr('');}}
              style={{padding:'24px 16px',background:'#f8fafc',border:'2px solid #e2e8f0',borderRadius:14,cursor:'pointer',textAlign:'center'}}>
              <div style={{fontSize:32,marginBottom:8}}>🎓</div>
              <div style={{fontWeight:800,fontSize:15,color:'#1e293b'}}>{t.student}</div>
              <div style={{fontSize:11,color:'#94a3b8',marginTop:4}}>{t.loginWithCode}</div>
            </button>
          </div>
        )}
        ...
        {role==='teacher'&&(
          <>
            <button onClick={()=>{setRole(null);setErr('');}} style={{background:'none',border:'none',color:'#94a3b8',cursor:'pointer',fontSize:13,marginBottom:16,padding:0}}>{t.back}</button>
            ...
            <input value={user} onChange={e=>setUser(e.target.value)} placeholder={t.username}
              style={{width:'100%',padding:'12px 14px',border:'2px solid #e2e8f0',borderRadius:10,fontSize:15,outline:'none',marginBottom:10,boxSizing:'border-box'}} />
            <input type="password" value={pass} onChange={e=>setPass(e.target.value)} placeholder={t.password} onKeyDown={e=>e.key==='Enter'&&handleTeacherLogin()}
              style={{width:'100%',padding:'12px 14px',border:'2px solid #e2e8f0',borderRadius:10,fontSize:15,outline:'none',marginBottom:16,boxSizing:'border-box'}} />
            <button onClick={handleTeacherLogin} disabled={loading}
        ...
        {role==='student'&&(
          <>
            <button onClick={()=>{setRole(null);setErr('');}} style={{background:'none',border:'none',color:'#94a3b8',cursor:'pointer',fontSize:13,marginBottom:16,padding:0}}>{t.back}</button>
            {err && <div style={{color:'#dc2626',fontSize:13,background:'#fef2f2',padding:'8px 12px',borderRadius:8,border:'1px solid #fecaca',marginBottom:14,textAlign:'center'}}>{err}</div>}
            <div style={{fontSize:13,color:'#64748b',marginBottom:12,textAlign:'center'}}>{t.teacherCode}</div>
            <input value={code} onChange={e=>setCode(e.target.value)} placeholder={t.code} onKeyDown={e=>e.key==='Enter'&&handleStudentLogin()}
              style={{width:'100%',padding:'14px',border:'2px solid #e2e8f0',borderRadius:10,fontSize:20,outline:'none',marginBottom:16,boxSizing:'border-box',textAlign:'center',letterSpacing:4,fontWeight:700}} />
...
function HomePage({exams, students, onSelectExam, currentExam, onDeleteExam, onNavigate, onEditExam, dark:d=false}) {
  const avgScaled = students.length?(students.reduce((a,b)=>a+b.scaled,0)/students.length).toFixed(1):'—';
  ...
  return (
    <div style={{maxWidth:1200,margin:'0 auto',padding:'24px 20px'}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(180px,1fr))',gap:12,marginBottom:24}}>
        {[['Нийт шалгалт',exams.length,'#dc2626'],['Нийт сурагч',students.length,'#8b5cf6'],['Дундаж ЭЕШ',avgScaled,'#22c55e'],['Шалгагдсан',students.length,'#f97316']].map(([label,val,c])=>(
          <div key={label} style={{background:'white',borderRadius:12,padding:'16px 20px',boxShadow:'0 1px 4px rgba(0,0,0,.08)'}}>
...
function RatingPage({exams, students, dark:d=false}) {
  ...
  const filtered = students
    .filter(s => filterExam==='all' || s.examId===filterExam)
    .filter(s => filterClass==='all' || (s.code||'').startsWith(filterClass));

  const sorted = [...filtered].sort((a,b)=>b.scaled-a.scaled);

  // Class grouping
  const allClasses = [...new Set(students.map(s=>s.code?.split('-')?.[0]||'').filter(Boolean))].sort();
  const classStats = allClasses.map(cls=>{
    const clsStudents = filtered.filter(s=>(s.code||'').startsWith(cls));
    if(!clsStudents.length) return null;
    const avg = clsStudents.reduce((s,x)=>s+x.scaled,0)/clsStudents.length;
    const top = [...clsStudents].sort((a,b)=>b.scaled-a.scaled)[0];
    return {cls, count:clsStudents.length, avg:avg.toFixed(1), top, students:clsStudents};
  ...
  return (
    <div style={{maxWidth:1000,margin:'0 auto',padding:'24px 20px'}}>
      <div style={{background:'linear-gradient(135deg,#7c3aed,#a855f7)',borderRadius:20,padding:'28px 28px',color:'white',marginBottom:24}}>
        <div style={{fontSize:11,letterSpacing:3,opacity:.7,marginBottom:6}}>EYESH CHECKER</div>
        <div style={{fontSize:28,fontWeight:900,marginBottom:8}}>🏆 Рейтинг</div>
        <div style={{opacity:.8,fontSize:13}}>Сурагчдыг оноогоор эрэмбэлсэн жагсаалт</div>
      </div>

      {/* Filters */}
      <div style={{background:'white',borderRadius:14,padding:16,marginBottom:16,boxShadow:'0 1px 4px rgba(0,0,0,.08)',display:'flex',gap:12,flexWrap:'wrap',alignItems:'center'}}>
        <div>
          <div style={{fontSize:11,fontWeight:700,color:'#94a3b8',marginBottom:6}}>ШАЛГАЛТ</div>
          <div style={{display:'flex',gap:4,flexWrap:'wrap'}}>
            <button onClick={()=>setFilterExam('all')} style={{padding:'5px 10px',borderRadius:7,border:'none',fontWeight:700,fontSize:12,cursor:'pointer',background:filterExam==='all'?'#7c3aed':'#f1f5f9',color:filterExam==='all'?'white':'#374151'}}>Бүгд</button>
            {exams.map(e=>(
              <button key={e.id} onClick={()=>setFilterExam(e.id)} style={{padding:'5px 10px',borderRadius:7,border:'none',fontWeight:700,fontSize:11,cursor:'pointer',background:filterExam===e.id?'#7c3aed':'#f1f5f9',color:filterExam===e.id?'white':'#374151',maxWidth:140,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{e.title}</button>
            ))}
          </div>
        </div>
        {allClasses.length>0&&<div>
          <div style={{fontSize:11,fontWeight:700,color:'#94a3b8',marginBottom:6}}>АНГИ</div>
          <div style={{display:'flex',gap:4,flexWrap:'wrap'}}>
            <button onClick={()=>setFilterClass('all')} style={{padding:'5px 10px',borderRadius:7,border:'none',fontWeight:700,fontSize:12,cursor:'pointer',background:filterClass==='all'?'#dc2626':'#f1f5f9',color:filterClass==='all'?'white':'#374151'}}>Бүгд</button>
            {allClasses.map(c=>(
              <button key={c} onClick={()=>setFilterClass(c)} style={{padding:'5px 10px',borderRadius:7,border:'none',fontWeight:700,fontSize:12,cursor:'pointer',background:filterClass===c?'#dc2626':'#f1f5f9',color:filterClass===c?'white':'#374151'}}>{c}</button>
            ))}
          </div>
        </div>}
        <div style={{marginLeft:'auto',display:'flex',gap:4}}>
          <button onClick={()=>setView('list')} style={{padding:'7px 14px',borderRadius:8,border:'none',fontWeight:700,fontSize:12,cursor:'pointer',background:view==='list'?'#1e293b':'#f1f5f9',color:view==='list'?'white':'#374151'}}>📋 Жагсаалт</button>
          <button onClick={()=>setView('class')} style={{padding:'7px 14px',borderRadius:8,border:'none',fontWeight:700,fontSize:12,cursor:'pointer',background:view==='class'?'#1e293b':'#f1f5f9',color:view==='class'?'white':'#374151'}}>🏫 Ангиар</button>
        </div>
      </div>

      {/* List view */}
      {view==='list'&&(
        <div style={{background:'white',borderRadius:14,boxShadow:'0 1px 4px rgba(0,0,0,.08)',overflow:'hidden'}}>
          {sorted.length===0?<div style={{textAlign:'center',padding:60,color:'#94a3b8'}}>Өгөгдөл байхгүй</div>:(
            sorted.map((s,i)=>{
              const grade=eyeshGrade(s.scaled);
              const exam=examMap[s.examId];
              const isMedal=i<3;
              return (
                <div key={s.id} style={{display:'flex',alignItems:'center',gap:12,padding:'12px 20px',borderBottom:'1px solid #f1f5f9',background:i===0?'#fffbeb':i===1?'#f8fafc':i===2?'#fef3e2':'white'}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontWeight:800,fontSize:14,color:'#1e293b',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{s.name}</div>
                    <div style={{fontSize:11,color:'#94a3b8'}}>{s.code||''}{exam?' · '+exam.title:''}</div>
                  </div>
                  <div style={{textAlign:'right'}}>
                    <div style={{fontSize:20,fontWeight:900,color:grade.c}}>{s.scaled}%</div>
                    <div style={{fontSize:10,fontWeight:700,color:grade.c,background:grade.c+'18',padding:'1px 6px',borderRadius:4,display:'inline-block'}}>{grade.l}</div>
                  </div>
                  <div style={{minWidth:40,textAlign:'center'}}>
                    <div style={{width:36,height:36,borderRadius:'50%',background:grade.c,color:'white',fontWeight:900,fontSize:14,display:'flex',alignItems:'center',justifyContent:'center'}}>{grade.g}</div>
                  </div>
                </div>
              );
            })
      ...
      {view==='class'&&(
        <div style={{display:'flex',flexDirection:'column',gap:12}}>
          {classStats.length===0?<div style={{background:'white',borderRadius:14,padding:60,textAlign:'center',color:'#94a3b8'}}>Өгөгдөл байхгүй</div>:
          classStats.map((cs,ci)=>(
            <div key={cs.cls} style={{background:'white',borderRadius:14,padding:20,boxShadow:'0 1px 4px rgba(0,0,0,.08)'}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
                <div style={{display:'flex',alignItems:'center',gap:10}}>
                  {ci<3&&<span style={{fontSize:24}}>{['🥇','🥈','🥉'][ci]}</span>}
                  <div>
                    <div style={{fontWeight:900,fontSize:16,color:'#1e293b'}}>{cs.cls} анги</div>
                    <div style={{fontSize:12,color:'#64748b'}}>{cs.count} сурагч</div>
                  </div>
                </div>
                <div style={{textAlign:'right'}}>
                  <div style={{fontSize:26,fontWeight:900,color:eyeshGrade(pf(cs.avg)).c}}>{cs.avg}%</div>
                  <div style={{fontSize:11,color:'#94a3b8'}}>дундаж</div>
                </div>
              </div>
              <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
                {[...cs.students].sort((a,b)=>b.scaled-a.scaled).slice(0,10).map((s,si)=>{
                  const g=eyeshGrade(s.scaled);
                  return (
                    <div key={s.id} style={{display:'flex',alignItems:'center',gap:6,padding:'5px 10px',background:g.c+'12',borderRadius:8,border:'1px solid '+g.c+'30'}}>
                      <span style={{fontSize:11,color:'#94a3b8'}}>{si+1}.</span>
                      <span style={{fontSize:12,fontWeight:700,color:'#1e293b'}}>{s.name}</span>
                      <span style={{fontSize:12,fontWeight:900,color:g.c}}>{s.scaled}%</span>
                    </div>
                  );
                })}
                {cs.students.length>10&&<div style={{padding:'5px 10px',fontSize:12,color:'#94a3b8'}}>+{cs.students.length-10} бусад</div>}
              </div>
            </div>
          ))}
        </div>
...
function BoardPage({exam, students, onDeleteStudent, onExportExcel, dark:d=false}) {
  ...
  function exportStudentPDF(student) {
    import('jspdf').then(({jsPDF})=>{
      ...
      doc.text('EYESH Checker — Дүнгийн Тайлан', pw, 12);
      doc.setFontSize(10); doc.setFont('helvetica','normal');
      doc.text(exam.title+' | '+exam.subject, pw, 20);
      doc.text(new Date().toLocaleDateString('mn-MN'), W-pw, 20, {align:'right'});
      // Student info box
      const grade = eyeshGrade(student.scaled||0);
      doc.setFillColor(248,250,252);
      doc.rect(pw,34,W-pw*2,36,'F');
      doc.setDrawColor(220,226,234);
      doc.rect(pw,34,W-pw*2,36,'S');
      doc.setTextColor(30,41,59);
      doc.setFontSize(18); doc.setFont('helvetica','bold');
      doc.text(student.name||student.code||'', pw+6, 46);
      doc.setFontSize(10); doc.setFont('helvetica','normal');
      doc.setTextColor(100,116,139);
      doc.text('Код: '+(student.code||''), pw+6, 54);
      doc.text('Анги: '+(student.class||'—'), pw+6, 60);
      // Score circle
      doc.setFillColor(...hexToRgb(grade.c));
      doc.circle(W-pw-20, 52, 16, 'F');
      doc.setTextColor(255,255,255);
      doc.setFontSize(18); doc.setFont('helvetica','bold');
      doc.text((student.scaled||0)+'%', W-pw-20, 55, {align:'center'});
      doc.setFontSize(9);
      doc.text(grade.l||'', W-pw-20, 62, {align:'center'});
      // Stats row
      let cx = pw;
      const stats = [['Зөв',student.correct,'#16a34a'],['Буруу',student.wrong,'#dc2626'],['Хоосон',student.blank,'#94a3b8'],['Нийт оноо',(student.rawEarned||0)+'/'+(student.rawMax||0),'#7c3aed']];
      doc.setFontSize(10);
      stats.forEach(([l,v,c])=>{
        const bw=(W-pw*2)/4;
        doc.setFillColor(...hexToRgb(c+'18'||'#f8fafc'));
        doc.rect(cx,76,bw-4,22,'F');
        doc.setTextColor(...hexToRgb(c));
        doc.setFont('helvetica','bold'); doc.setFontSize(14);
        doc.text(String(v), cx+bw/2-2, 87, {align:'center'});
        doc.setTextColor(100,116,139); doc.setFont('helvetica','normal'); doc.setFontSize(8);
        doc.text(l, cx+bw/2-2, 93, {align:'center'});
        cx+=bw;
      });
      // Section 1 answers
      let y=108;
      doc.setFillColor(30,41,59);
      doc.rect(pw,y-6,W-pw*2,10,'F');
      doc.setTextColor(255,255,255); doc.setFontSize(10); doc.setFont('helvetica','bold');
      doc.text('1-р хэсгийн хариулт', pw+4, y);
      y+=10;
      const results = student.sec1Results||[];
      const cols = 6;
      const cw = (W-pw*2)/cols;
      results.forEach((r,i)=>{
        const col=i%cols; const row=Math.floor(i/cols);
        const x=pw+col*cw; const ry=y+row*10;
        if(ry>275){return;}// page overflow guard
        const bg=r.st==='ok'?'#dcfce7':r.st==='blank'?'#f1f5f9':'#fee2e2';
        const tc=r.st==='ok'?'#15803d':r.st==='blank'?'#94a3b8':'#dc2626';
        doc.setFillColor(...hexToRgb(bg));
        doc.rect(x,ry-1,cw-2,8,'F');
        doc.setTextColor(...hexToRgb(tc)); doc.setFont('helvetica','bold'); doc.setFontSize(7);
        doc.text('D'+(i+1)+': '+(r.pts||0)+'пт', x+1, ry+4);
      });
      // Sec2 if exists
      const sec2Rows = Object.entries(student.sec2Results||{});
      if(sec2Rows.length>0){
        y=y+Math.ceil(results.length/cols)*10+8;
        doc.setFillColor(124,58,237);
        doc.rect(pw,y-6,W-pw*2,10,'F');
        doc.setTextColor(255,255,255); doc.setFontSize(10); doc.setFont('helvetica','bold');
        doc.text('2-р хэсгийн хариулт', pw+4, y);
        y+=10;
        sec2Rows.forEach(([sub,rows])=>{
          doc.setTextColor(124,58,237); doc.setFont('helvetica','bold'); doc.setFontSize(9);
          doc.text(sub, pw, y+4); y+=8;
          let sx=pw+10;
          Object.entries(rows).forEach(([row,r])=>{
            const bg=r.st==='ok'?'#dcfce7':r.st==='blank'?'#f1f5f9':'#fee2e2';
            const tc=r.st==='ok'?'#15803d':r.st==='blank'?'#94a3b8':'#dc2626';
            doc.setFillColor(...hexToRgb(bg));
            doc.rect(sx,y-1,22,8,'F');
            doc.setTextColor(...hexToRgb(tc)); doc.setFont('helvetica','bold'); doc.setFontSize(7);
            doc.text(row+':'+(r.pts||0), sx+2, y+4);
            sx+=24; if(sx>W-pw-22){sx=pw+10;y+=10;}
          });
          y+=12;
        });
      }
      ...
      doc.text('EYESH Checker — AI Bubble Detection System', pw, 290);
      doc.text('ЭЕШ Зэрэглэл: A≥90 Онц | B≥80 Сайн | C≥70 Дунд сайн | D≥60 Хангалттай | F<60 Тэнцээгүй', pw, 294);
      doc.save((student.name||student.code)+'_'+exam.title+'_дүн.pdf');
    });
  }
...
export default function App() {
  ...
  if (!authed && !studentMode) return (
    <LoginPage
      onLogin={()=>{setAuthed(true);loadData();}}
    ...
    <>
      ...
      <header style={{background:d?'#1e293b':'white',borderBottom:'1px solid '+(d?'#334155':'#e2e8f0'),position:'sticky',top:0,zIndex:100,boxShadow:'0 2px 8px rgba(0,0,0,.06)'}}>
        <div style={{maxWidth:1200,margin:'0 auto',padding:'0 20px',display:'flex',alignItems:'center',gap:12,height:60}}>
          ...
          <button onClick={()=>{localStorage.removeItem('eyesh_token');setAuthed(false);}}
